"""Contenido del DOCUMENTO FUNCIONAL SCTIS — Beneficios y Descripción para Decisores."""
...
SISTEMA = "SCTIS v1.0"
DOC_CODIGO = "SCTIS-DOCFUN-001"
FECHA = "Agosto 2026"

BLOCKS = [
    ("h1", "1. Introducción"),
    ("p", "El Sistema de Gestión de Tiras de Interrupción Eléctrica (SCTIS) automatiza el proceso de recolección, validación y consolidación de las tiras de interrupción de servicio que reportan los estados operadores de CORPOELEC.\n\nHoy, cada estado prepara sus reportes en archivos Excel con formatos propios, los envía por correo y el equipo de seguimiento los consolida manualmente. Ese proceso es lento, propenso a errores y dificulta la trazabilidad.\n\nCon SCTIS, el operador de cada estado sube su archivo Excel al sistema web, y este valida automáticamente la estructura, homologa las causas a las oficiales, verifica que las subestaciones y circuitos existan, detecta duplicados, mide la calidad de los datos y deja todo registrado para auditoría. El resultado es información confiable, disponible en tiempo real y respaldada por tableros de control."),

    ("h2", "1.1. Párrafo ejecutivo"),
    ("info", "SCTIS convierte un proceso manual, descentralizado y propenso a errores en una plataforma centralizada, automatizada y auditable. Lo que antes tomaba días de trabajo y revisiones, ahora se hace en minutos con controles de calidad automáticos. El sistema fue desarrollado con asistencia de modelos de inteligencia artificial y sienta las bases para una futura migración a la nube (Supabase) y a Google AI Studio.", "EN LENGUAJE CLARO"),

    ("h1", "2. El Problema Actual (Antes)"),
    ("table",
    ...
    ("h1", "5. Descripción Funcional del Software"),
    ("h2", "5.1. Acceso y roles"),
    ("p", "El sistema se accede por un navegador web con usuario y contraseña. Existen dos roles: el administrador (accede a todos los estados y gestiona usuarios, auditoría y tareas) y el operador/editor (solo ve y carga su estado asignado). La seguridad limita el acceso a nivel de base de datos: un operador no puede ver ni modificar datos de otros estados."),

    ("h2", "5.2. Carga de datos desde Excel"),
    ("p", "El operador sube su archivo Excel desde la opción 'Importar'. Si el archivo tiene varias hojas, el sistema le pide que seleccione una sola. Luego procesa la hoja y le muestra un resumen: cantidad de registros, formato detectado, causas a homologar y activos sin resolver. El operador revisa y confirma; solo entonces los datos se guardan en la base de datos."),

    ("h2", "5.3. Formato inteligente"),
    ("p", "El sistema reconoce automáticamente el formato de cada estado comparando los encabezados de la hoja contra un catálogo de 11 formatos. Si el archivo no coincide con ninguno, lo rechaza con un mensaje claro de qué está mal y cómo corregirlo."),

    ("h2", "5.4. Homologación de causas"),
    ("p", "Cada estado reporta las causas a su manera. El sistema las asigna automáticamente a las 22 causas oficiales del catálogo, mostrando el resultado en la previsualización para que el operador lo confirme o ajuste. Cuando hay ambigüedad, la inteligencia artificial sugiere la causa más probable."),

    ("h2", "5.5. Resolución de activos"),
    ("p", "El sistema verifica que cada subestación y circuito del archivo exista en el inventario oficial del estado. Si un nombre no coincide (por ejemplo por diferencias de tildes o numerales), lo marca y ofrece opciones de emparejamiento. El sistema aprende de cada decisión: cuando el operador señala a qué activo se refiere, guarda esa referencia y en las cargas siguientes el nombre se resuelve automáticamente."),
    ("p", "Si un activo no está en el inventario, el operador puede reportarlo como nuevo. La tira se carga igual (sin bloqueos), y la solicitud queda en una bandeja de revisión para que el administrador la apruebe, la asocie a un activo existente o la rechace. Así el inventario se mantiene íntegro y gobernado."),

    ("h2", "5.6. Control de calidad"),
    ("p", "Cada registro recibe un puntaje de calidad (0-100) basado en completitud, consistencia y precisión. Los duplicados se detectan automáticamente. Los registros con problemas no se descartan, pero quedan marcados para revisión, porque en el sector eléctrico la información disponible suele ser la única que existe."),

    ("h2", "5.7. Auditoría y tareas"),
    ("p", "Cada carga queda registrada con usuario, fecha, archivo, hoja, cantidad de aceptados y rechazados. Si hay registros rechazados, el sistema genera automáticamente una tarea de corrección y un archivo Excel con los errores por fila para que el operador los revise y recargue."),

    ("h2", "5.8. Tablero de control"),
    ("p", "El dashboard presenta 4 perspectivas: Ingeniero Eléctrico (causas, horas, tendencia), Mantenimiento, Project Manager y Data Scientist. Incluye gráficos de causas, horas, tendencias, sistemas, circuitos, subestaciones, estados y horarios, más un diagrama Sankey que desglosa las causas 'OTRAS' en su causa original."),

    ("h2", "5.9. Aprendizaje del sistema y supervisión de activos"),
    ("p", "El SCTIS incorpora dos mecanismos complementarios para mantener íntegro el inventario maestro de subestaciones y circuitos:"),
    ("bullets", [
    ...
    ("p", "Todo queda registrado: quién reportó, quién decidió, cuándo y con qué comentario. Este control de cambios es clave para la trazabilidad y la confiabilidad de los datos bajo normas ISO."),

    ("h2", "5.10. Módulo de Monitoreo de Cargas y Matriz de Seguimiento"),
    ("p", "El sistema cuenta con un Módulo Especializado de Monitoreo de Cargas que elimina la incertidumbre sobre el cumplimiento de entregas de datos por parte de los 24 estados operadores de Venezuela:"),
    ("bullets", [
    ...
    ("p", "Todo el proceso de seguimiento queda completamente documentado y automatizado sin dejar espacio a dudas sobre quién falta por entregar su información."),

    ("pagebreak",),

    ("h1", "6. Diagrama de Flujo General"),
    ("flow", [
    ...
    ("p", "El proceso completo, desde subir el archivo hasta ver los datos en el tablero, se realiza en la misma sesión web y queda registrado para auditoría."),

    ("h1", "7. Impacto Esperado"),
    ("table",
    ...
    ("h1", "9. Desarrollo con Inteligencia Artificial"),
    ("p", "El SCTIS fue desarrollado íntegramente con asistencia de modelos de inteligencia artificial, desde el análisis de formatos y el diseño del esquema de datos hasta la generación de código y documentación. En producción, la IA (Google Gemini, modelo gemini-2.5-flash) apoya la homologación de causas, la evaluación de calidad y la detección de duplicados. La fase siguiente migra esta capacidad a Google AI Studio."),
    ("info", "Este proyecto demuestra que la IA aplicada al sector eléctrico venezolano permite automatizar procesos complejos, reducir errores y liberar tiempo del personal técnico para tareas de mayor valor. Los modelos de asistencia al desarrollo serán listados en detalle por el equipo.", "IA APLICADA AL SEN"),
    ...
    ("h1", "Anexo B — Catálogo de Formatos"),
