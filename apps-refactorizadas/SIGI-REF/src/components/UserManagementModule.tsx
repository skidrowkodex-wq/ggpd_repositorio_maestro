...
import { useAuth, getCatalogUsers, saveCatalogUsers } from '../context/AuthContext';
import { 
...
export const UserManagementModule: React.FC = () => {
  ...
  const [usersList, setUsersList] = useState<InstitutionalUser[]>(() => getCatalogUsers());
  const [isLoadingInsForge, setIsLoadingInsForge] = useState<boolean>(false);
  const [isSyncingInsForge, setIsSyncingInsForge] = useState<boolean>(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedState, setSelectedState] = useState<string>('ALL');
  const [selectedRole, setSelectedRole] = useState<string>('ALL');
  const [activeTab, setActiveTab] = useState<'USERS' | 'REQUESTS' | 'AUDIT_LOGS'>('USERS');
  
  // Cargar usuarios vivos de InsForge en montaje
  useEffect(() => {
    setIsLoadingInsForge(true);
    fetchUsersFromInsForge()
      .then(remoteUsers => {
      ...
      .finally(() => setIsLoadingInsForge(false));
  }, []);

  const handleSyncInsForge = async () => {
    setIsSyncingInsForge(true);
    try {
  ...
  const saveWizardPermission = async () => {
    ...
    const nextPermissions = {
      ...wizardUser.permissions,
      [wizardAppKey]: wizardGranted,
    };

    // Actualizar en base de datos InsForge
    updateUserPermissionsInInsForge(wizardUser.id, nextPermissions);
    ...
    const newLog = logSecurityAuditEvent({
      ...
      details: wizardAuditNote || `Acceso a ${APPS_META[wizardAppKey]?.shortName} ${wizardGranted ? 'otorgado' : 'revocado'} por ${session.name} en InsForge`,
  ...
  // Toggle User Active / Suspended (Kill-switch)
  const toggleUserStatus = async (user: InstitutionalUser) => {
    const nextStatus: 'ACTIVO' | 'SUSPENDIDO' = user.status === 'SUSPENDIDO' ? 'ACTIVO' : 'SUSPENDIDO';
    toggleUserStatusInInsForge(user.id, nextStatus);

    // Si el usuario es SUSPENDIDO y tiene correo Google asociado, revocar acceso en Data Lake vía Webhook
    if (nextStatus === 'SUSPENDIDO' && user.googleEmail) {
      dispatchGoogleDriveWebhook('REVOKE', {
        email: user.googleEmail,
        adminName: session.name,
        reason: `Kill-Switch Activado: Usuario suspendido por ${session.name} (${session.role})`,
      });
    }

    setUsersList(prev => {
      const updated: InstitutionalUser[] = prev.map(u => u.id === user.id ? { ...u, status: nextStatus } : u);
      saveCatalogUsers(updated);
      return updated;
    });

    const newLog = logSecurityAuditEvent({
      eventType: nextStatus === 'SUSPENDIDO' ? 'USER_SUSPENDED' : 'USER_REACTIVATED',
      userId: user.id,
      username: user.username,
      fullName: user.fullName,
      details: `Estatus de usuario cambiado a ${nextStatus} por ${session.name} (${session.role}) en InsForge IAM y Data Lake.`,
  ...
  const handleSaveUserForm = async () => {
    ...
    if (editingUser) {
      const updatedUser: InstitutionalUser = {
        ...
        googleEmail: sanitizedGoogleEmail || editingUser.googleEmail,
      } as InstitutionalUser;

      // Guardar en InsForge en segundo plano
      saveUserToInsForge(updatedUser, formUser.initialPassword);

      setUsersList(prev => {
        const updated = prev.map(u => u.id === editingUser.id ? updatedUser : u);
        saveCatalogUsers(updated);
        return updated;
      });
      setEditingUser(null);
      ...
      // Guardar en InsForge
      saveUserToInsForge(newUser, newUser.initialPassword);
      ...
      const createdLog = logSecurityAuditEvent({
        ...
        details: `Nuevo usuario creado por ${session.name} con rol ${newUser.role} en InsForge. Correo Corp: ${newUser.email}, Google: ${newUser.googleEmail || 'N/A'}`,
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#072146] via-[#002b49] to-[#041426] text-white p-6 sm:p-7 shadow-xl border border-blue-900/60 dark:border-[#00f2fe]/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 group hover:border-[#00f2fe]/80 transition-all duration-300">
        ...
        <div className="relative z-10 shrink-0 flex items-center gap-2">
          <button
            ...
            disabled={isSyncingInsForge || isLoadingInsForge}
            title="Sincronizar usuarios en tiempo real con la base de datos canónica InsForge"
            className="flex items-center space-x-2 rounded-2xl bg-blue-900/50 hover:bg-blue-800/70 border border-cyan-400/40 text-cyan-200 px-4 py-3 text-xs font-bold uppercase shadow-md transition-all shrink-0 disabled:opacity-50"
          >
            <RefreshCw className={`h-4 w-4 text-cyan-300 ${isSyncingInsForge || isLoadingInsForge ? 'animate-spin' : ''}`} />
            <span>{isSyncingInsForge ? 'Sincronizando...' : 'Sincronizar InsForge'}</span>
          </button>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          <button
                            ...
                            title={user.status === 'SUSPENDIDO' ? 'Reactivar Usuario en InsForge' : 'Suspender Usuario (Kill-Switch)'}
                          ...
                          >
                            {user.status === 'SUSPENDIDO' ? <UserCheck className="h-4 w-4" /> : <UserX className="h-4 w-4" />}
                          </button>
