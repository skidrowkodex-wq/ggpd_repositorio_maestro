...
import { ShieldCheck, Cpu, Sparkles, Lock, ExternalLink, ShieldAlert, Award, CheckCircle2, Info, HelpCircle } from 'lucide-react';
      ...
      <footer className="w-full border-t border-slate-300 dark:border-[#00f2fe]/30 bg-slate-100 dark:bg-[#040814] text-slate-700 dark:text-slate-300 pt-10 pb-6 mt-16 transition-colors">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 space-y-8">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="font-black text-slate-900 dark:text-white">
                  SIGI · Sistema Integrado de Gestión de la Información
                </span>
                <button 
                  onClick={() => setIsSigiModalOpen(true)}
                  className="inline-flex items-center space-x-1 text-[11px] font-bold text-[#002b49] dark:text-[#00f2fe] hover:underline cursor-pointer"
                >
                  <Info className="h-3.5 w-3.5" />
                  <span>¿Qué es SIGI?</span>
                </button>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                CORPOELEC — Gerencia General de Planificación de Distribución (GGPD). Órgano adscrito al Ministerio del Poder Popular para la Energía Eléctrica (MPPEE).
              </p>
            </div>
          ...
          <div className="text-center pt-4 border-t border-slate-300 dark:border-slate-800 text-[10px] text-slate-500 space-y-1">
            <p>© 2026 Corporación Eléctrica Nacional (CORPOELEC) · <strong>SIGI (Sistema Integrado de Gestión de la Información)</strong>. Todos los derechos reservados.</p>
