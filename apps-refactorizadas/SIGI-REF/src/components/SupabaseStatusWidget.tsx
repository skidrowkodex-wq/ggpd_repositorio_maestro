...
import { testInsforgeConnection, InsForgeConnectionTestResult, insforgeUrl, insforgeProjectId } from '../services/insforgeClient';
...
export const SupabaseStatusWidget: React.FC = () => {
  const [status, setStatus] = useState<InsForgeConnectionTestResult | null>(null);
  const [loading, setLoading] = useState<boolean>(true);

  const runDiagnostic = async () => {
    setLoading(true);
    try {
      const res = await testInsforgeConnection();
      setStatus(res);
    } catch (e: any) {
      setStatus({
        success: false,
  ...
  const endpointHost = insforgeUrl.replace(/^https?:\/\//, '');
    ...
    <div className="rounded-3xl bg-white dark:bg-[#081224] p-5 border border-slate-200 dark:border-slate-800 shadow-sm transition-all space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h3 className="text-sm font-black text-slate-900 dark:text-white">Conexión a Base de Datos InsForge</h3>
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">
              Endpoint: <code className="font-mono text-emerald-800 dark:text-emerald-300 font-bold">{endpointHost}</code>{' '}
              <span className="text-slate-400">({insforgeProjectId})</span>
            </p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex items-center space-x-3 p-3 rounded-2xl bg-slate-50 dark:bg-[#0a192f] border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300 font-medium">
          ...
          <span>Estableciendo handshake y validando conexión con InsForge BaaS Postgres...</span>
        </div>
      ) : status?.success ? (
        <div className="space-y-3">
          <div className="flex items-start space-x-3 p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/30 text-xs text-emerald-900 dark:text-emerald-200 font-medium">
            ...
            <div>
              ...
              <span className="text-[10px] text-emerald-800 dark:text-emerald-400 block mt-0.5">
                URL Activa: {status.urlUsed} • Proyecto: <code className="font-mono font-bold">{status.projectId}</code> • RLS (Row-Level Security) habilitado según directiva ISO 27001.
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
            ...
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-[#081427] border border-slate-200 dark:border-slate-800">
              ...
              <span className="font-extrabold text-slate-900 dark:text-white">PostgreSQL 16</span>
            </div>
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-[#081427] border border-slate-200 dark:border-slate-800">
              <span className="text-[9px] text-slate-500 font-bold uppercase block">Esquemas</span>
              <span className="font-extrabold text-[#d97706] dark:text-[#ffd700]">core, public</span>
            </div>
          </div>
        </div>
      ) : (
        <div className="flex items-start space-x-3 p-3.5 rounded-2xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-500/30 text-xs text-red-900 dark:text-red-200 font-medium">
          ...
          <div>
            <span className="font-black text-red-900 dark:text-red-300 block">No se pudo conectar a InsForge</span>
